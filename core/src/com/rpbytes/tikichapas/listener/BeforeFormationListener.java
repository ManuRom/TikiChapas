 ...
 */
public class BeforeFormationListener extends CarouselFormationListener{

    public BeforeFormationListener(Cell cell, Array<Formation> formations, MatchSettings matchSettings, Skin skin , boolean home){
    ...
    public void clicked(InputEvent event, float x, float y) {
        for (int i=0;i<getFormations().size;i++) {
            if(getFormations().get(i).getMainImage() == ((Table) getCell().getActor()).getCells().peek().getActor()){
                if(i == 0) {
                    ((Table)getCell().getActor()).getCells().first().setActor(new Label(getFormations().get(getFormations().size-1).getName(),getSkin(), SkinKeys.LABEL));
                    ((Table)getCell().getActor()).getCells().peek().setActor(getFormations().get(getFormations().size-1).getMainImage());
                    if(isHome())
                        getMatchSettings().setHomeFormation(getFormations().get(getFormations().size-1));
                    else
                        getMatchSettings().setAwayFormation(getFormations().get(getFormations().size-1));
                    break;
                }else{
